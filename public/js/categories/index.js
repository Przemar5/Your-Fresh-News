$(document).ready(function () {
    ...
    let createRow = (data) => {
        let url = 'https://yourfreshnews.herokuapp.com/categories/'
        let routeShow = url + data.category.slug
        let routeEdit = url + data.category.slug + '/edit'
        let routeUpdate = url + data.category.id
        let routeDelete = url + data.category.id
    ...
    $('body').on('submit', '.form-edit', function (e) {
        ...
        for (i in requiredColumns) {
            ...
            category[requiredColumns[i].name] = tds[i].text().trim()
        }
        ...
        if (! $(this).hasClass('edit-mode')) {
            ...
            tds[1].append('<input type="text" name="slug" ' + 
                'class="form-control input-sm" value="' + category.slug + '" ' +                 'maxlength="255" data-parsley-length="255" ' + 
            'data-parsley-pattern="[\\w\\-]+" required ' + 
	...
	$('body').on('submit', '.form-delete', function (e) {
		...
		let categoryName = row.children('td').eq(1).text().trim()

		if (confirm('Are You sure you want remove category: "' + categoryName + '"?')) {
			$.ajaxSetup({
			    headers: {
			        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			    }
